...
import {
    sectionValidateErrorText,
    validateSectionData,
} from '../../util/validation/sectionValidate';
...
const CreateSection = ({ cancel, classId, startLoading, stopLoading }) => {
    ...
    const updateAssignment = (assignmentId, fieldName, data, assignmentIndex) => {
        ...
        let tempUpdatedAssignmentList = [...updatedAssigmentList];
        if (assignmentId) {
            index = tempUpdatedAssignmentList.findIndex(
            ...
            index = tempUpdatedAssignmentList.findIndex(
                (assignment) => assignment.assignmentIndex === assignmentIndex
            );
            if (index !== -1) {
                tempUpdatedAssignmentList[index][fieldName] = data;
            } else {
        ...
        return newUploadedDocuments.map((document, index) => {
            return (
                <DocumentFile
                    ...
                    index={index}
                    removeFunc={removeNewDocumentFile}
                />
            );
        });
    };
    const generateNewAssignmentList = () => {
        return newAssignmentList.map((assignment, index) => {
            let updatedIndex = updatedAssigmentList.findIndex(
                (assignment) => assignment.assignmentIndex === assignment.index
            );
            let isError = false;
            if (updatedIndex !== -1) {
            ...
            return (
                <EditAssignment
                    ...
                    isNew={true}
                    updateFunc={updateAssignment}
                    index={index}
                    removeFunc={removeAssignment}
                    isError={isError}
                />
            );
        });
        ...
        <ScrollView style={{ flex: 1 }}>
            <View style={styles.container}>
                <View style={styles.classDetailContainer}>
                    ...
                    <TextInput
                        value={sectionName}
                        style={[styles.input, error.sectionName ? styles.errorTextInput : null]}
                        placeholder='Enter section name'
                        onChangeText={(text) => setSectionName(text)}
                    />
                    {error.sectionName ? (
                        <Text style={{ color: '#d4380d' }}>
                            **{sectionValidateErrorText.MISS_SECTION_NAME}
                        </Text>
                    ) : (
                        <></>
                    )}
                ...
                <View style={styles.classDetailContainer}>
                    ...
                    <TextInput
                        ...
                        placeholder='Enter section name'
                        onChangeText={(text) => setSectionDetail(text)}
                    />
                </View>
                ...
                <Pressable style={styles.uploadButton} onPress={addAssignment}>
                    ...
                    <Text style={styles.uploadButtonText}>Add new Assignment</Text>
                </Pressable>
